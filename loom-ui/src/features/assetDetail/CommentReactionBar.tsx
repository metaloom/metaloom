...
/**
 * Compact thumbs up/down reaction bar for a single comment. Loads the comment's
 * reactions on mount and lets the current user toggle their own THUMBSUP /
 * THUMBSDOWN reaction (create if absent, delete if present).
 */
export function CommentReactionBar({
  ...
  useEffect(() => {
    if (!token || !commentUuid) {
      setReactions([]);
      return;
    }
    let active = true;
    listCommentReactions(token, commentUuid)
  ...
  const toggle = useCallback(
    async (type: TaskReactionType) => {
      if (!token || !commentUuid) return;
      const own = mine(type);
      try {
        if (own) {
          await deleteCommentReaction(token, commentUuid, own.uuid);
          setReactions((prev) => prev.filter((r) => r.uuid !== own.uuid));
        } else {
          const created = await createCommentReaction(token, commentUuid, { type });
    ...
    [token, commentUuid, mine, showToast, t],
  );
  ...
  return (
    <Box sx={{ display: "flex", gap: 0.5, mt: 0.5 }}>
      {button("THUMBSUP", t("reaction.thumbsUp"), "comment-reaction-up", ThumbUpAltOutlined)}
      {button("THUMBSDOWN", t("reaction.thumbsDown"), "comment-reaction-down", ThumbDownAltOutlined)}
    </Box>
  );
}
