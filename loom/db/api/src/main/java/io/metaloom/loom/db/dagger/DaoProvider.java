...
public interface DaoProvider extends DaoCollection {
	...
	default FailureReportDao failureReportDao() {
		return daos().failureReportDao();
	}

	default ShareDao shareDao() {
		return daos().shareDao();
	}

	default ShareFeedbackDao shareFeedbackDao() {
		return daos().shareFeedbackDao();
	}
