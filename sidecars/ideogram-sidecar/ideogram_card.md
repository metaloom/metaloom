...
license_link: https://huggingface.co/ideogram-ai/ideogram-4-nf4/blob/main/LICENSE.md
...
This model is integrated with the 🧨 `diffusers` library
...
#### diffusers - remote prompt upsampling 
...
#### diffusers - local prompt upsampling

For a full-local experience, diffusers ships a `prompt_upsampling` that utilizes the same `Qwen3-VL-8B` model as the text-encoder for the upsampling. 
Expect a quality decrease compared to remote prompt uspsampling 
