...
`DEPTH_HOST` / `DEPTH_PORT`, `MAGEFLOW_HOST` / `MAGEFLOW_PORT`). See the
[Cortex Helm chart](../helm/cortex) for deploying workers.

One caveat specific to the image sidecars: they are far heavier than the rest. Mage-Flow holds 17.5 GB
of bf16 weights and peaks near 20 GB, so it needs a dedicated ≥24 GB GPU rather than a share of one
that also runs `whisper` or `depthmap`.
