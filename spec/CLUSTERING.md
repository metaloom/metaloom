...
| **`runRegistry.get(...) == null`** | 🔴 Means "not owned *by this process*", **not** "not running". Any code treating it as the latter (as `LeaseReaper` does) is broken under N>1. `ProcessorEndpoint#resolveEngine` reads the same way, so under N>1 a worker's `TASK_RETURNED` for a run owned by another instance is silently dropped and that task waits out its lease after all. |
