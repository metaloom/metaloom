...
| `V2.80__collection_asset_cascade` | `collection_asset.collection_uuid` gets `ON DELETE CASCADE`. 🔴 It never had one: V2.8 created both FKs as plain references and V2.73 replaced only the asset-side one, while asserting in its own comment that the collection side was "already handled". Deleting a collection with **any** member therefore failed with a foreign-key violation - a 500 - and nothing noticed because membership had no REST surface, so the only collections anyone deleted were empty. The library side still blocks deliberately: a library must not be deleted out from under the assets in it |
