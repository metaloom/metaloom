...
| `/chats/:uuid/attachments` | POST (multipart), GET list | `ChatAttachmentEndpoint` | Files dropped into a chat. Owned by the conversation, deleted with it, never catalogued — see [../chat/LOOM_UI_CHAT.md](../chat/LOOM_UI_CHAT.md) §4.5 |
| `/chats/:uuid/attachments/:auuid` | DELETE | `ChatAttachmentEndpoint` | Detach a file |
| `/chats/:uuid/attachments/:auuid/data` | GET | `ChatAttachmentEndpoint` | Raw bytes (`addDownloadRoute`), `Content-Disposition: inline` |
| `/chats/:uuid/attachments/:auuid/asset` | POST | `ChatAttachmentEndpoint` | **Save to library** — ingests the file as a real asset. Optional query parameter `libraryUuid` |
...
| `/assets/:uuid/tag-placements/:placementUuid` | **PUT**, DELETE | PUT moves **one** placement to a different region — the write behind dragging a region tag on the video timeline. `TAG_ASSET`; body is `{area}` and a field it leaves unset keeps its stored value, so one edge can move without restating the other. Deliberately not untag-and-retag: that would mint a new `placementUuid` and record the dragger as the attacher. DELETE removes one placement, keeping the tag's others. `UNTAG_ASSET`. Both answer 404 when the placement belongs to another asset |
...
| `/assets/:uuid/media-token` · `/poster` · `/stream` | POST · GET · GET | Derived media, all `READ_ASSET_BINARY`. The poster and stream routes — and **only** those two — also accept the minted token as `?mt=`, because an `<img>`/`<video>` cannot set a header |
| `/assets/:uuid/media-info` · `/stream-start` | GET · GET | Measurements, session-authenticated. `media-info` is the ffprobe a timeline's length comes from; `stream-start?t=` answers where a stream requested at `t` will *actually* begin — a stream copy starts on a keyframe, and a player that assumes otherwise runs a clock seconds fast |
