...
_Last updated: 2026-08-09 (`GET /api/v1/metrics` — the JSON read of the `loom_*` meter catalog on the app port, gated by `READ_METRIC`; §4.3. Earlier: 2026-08-05 (tag placements: `V2.71` lets one tag sit on an asset several times, so `TagReference` now carries `placementUuid` and the provenance of the placement, and `DELETE /assets/:uuid/tag-placements/:placementUuid` removes one of them. Earlier the same day: `PUT /assets/:uuid/tags`, the bulk tagging route: one transaction, withdrawal by uuid, and the first route whose required permission set depends on the request. Earlier: pipeline node re-execution route, with the first per-option validation in the API. Earlier: the run-item `/tasks` and preview routes, and the four breakpoint/stepping routes. Earlier: annotation comment sub-resource; role `permissions` persisted and returned)_
