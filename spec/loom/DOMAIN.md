...
| **Video** | `asset_video_comp` | `stream_index` | `media_duration` (ms), `video_bitrate/encoding`, `fps`, `frame_count`, `rotation`, `blurriness`. **Written by `GET /assets/:uuid/media-info`** under `node_kind='probe'` since 2026-09-20 — the endpoint persists what ffprobe measured, so the second read of an asset answers from the column. Still no *node* writes it at ingest, which is why the first read of any video is a subprocess | V2.18 → V2.38 |
...
| **Cluster** | `cluster`, `embedding_cluster`, `tag_cluster`, `collection_cluster` | A producer's proposal that some embeddings are one subject, carrying a human verdict. `type` (e.g. `face`), `name` **nullable** — a machine proposal has none until somebody supplies one — UNIQUE per `(type, name)` when set. Review state: `status cluster_status` (PENDING/CONFIRMED/REJECTED, default PENDING), `person_uuid` (SET NULL), `reviewed_at`, `reviewer_uuid`. Provenance: `node_kind`, `node_id`, `producer_version`, `run_uuid`, `task_uuid`. Scope: `asset_uuid` **nullable on purpose** so a phase-2 library-wide cluster can live in the same table, plus `cluster_index`; UNIQUE `(asset_uuid, node_kind, cluster_index)` is the upsert key, and `upsertCluster` preserves the four review columns on conflict — which is why attributing clusters to a person survives a re-run and moving members between them would not. Ranking: `score` (mean cosine to the centroid, NULL for a singleton), `centroid real[]`, `model`, `dimensions` — a centroid is only comparable within one `(type, model, dimensions)`. `embedding_cluster` carries `origin` (AUTO/MANUAL), `confidence`, `created`. | → Asset (CASCADE), → Person (SET NULL); ← Embedding; ↔ Tag, Collection | V2.12 → rewritten V2.79; reviewer pair V2.88 |
