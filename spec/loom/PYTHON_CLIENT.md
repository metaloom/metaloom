...
| 38 mixin classes mirroring the Java `*Methods` interfaces | A rename lands in one file on each side, and the parity test compares the two trees mechanically. Cost: 226 methods on one object. |
...
│   ├── methods/         38 modules, one per Java *Methods interface
...
| `ALL_METHOD_GROUPS` | `loom_client/methods/__init__.py` | The 38 mixins, mirroring Java `ClientMethods`. |
