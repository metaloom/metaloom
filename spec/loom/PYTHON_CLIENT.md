...
| 39 mixin classes mirroring the Java `*Methods` interfaces | A rename lands in one file on each side, and the parity test compares the two trees mechanically. Cost: 248 methods on one object. |
...
_Last updated: 2026-08-05 (`bulk_tag_asset` and `remove_tag_placement` mirror the new Java methods; counts follow: 248 Java methods, 152 documented paths, 18 asset sub-resource methods that reject a hash)_
