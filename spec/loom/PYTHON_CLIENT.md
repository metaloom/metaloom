...
has 152 paths and exactly **one** `components.schemas` entry — request and response
...
| 44 mixin classes mirroring the Java `*Methods` interfaces | A rename lands in one file on each side, and the parity test compares the two trees mechanically. Cost: 301 methods on one object. |
...
│   ├── methods/         44 modules, one per Java *Methods interface
...
| `ALL_METHOD_GROUPS` | `loom_client/methods/__init__.py` | The 44 mixins, mirroring Java `ClientMethods`. |
...
overloads of `tagAsset`, `bulkTagAsset`, `assignTaskToAsset` and friends build paths
that cannot succeed. `LoomClient._asset_sub()` rejects a hash with an explanation
instead; 18 methods use it.
...
- [x] All 44 method groups, 301 methods — including `StorageMethods` (`load_storage_report`,
      `load_storage_backends`) and the eight account-picture methods on `UserMethods`
...
_Last updated: 2026-08-10 (`storage.py` mirrors the new `StorageMethods`, and `user.py` grew the
eight account-picture methods — `load/upload/download/delete_user_avatar` and their `my_`
counterparts. `EXPECTED_JAVA_METHOD_COUNT` 291 → 301; the two upload methods join `EXTRA_KWARGS` in
`tests/test_parity.py`, since the probe cannot build a multipart request without a filename. Earlier:
2026-08-05 (`bulk_tag_asset` and `remove_tag_placement` mirror the new Java methods))_
