...
| System log | The log panel at the bottom of the canvas folds from its own title bar (`pipeline-log-toggle`, `aria-expanded`), and **the title bar stays**. Collapsing used to take the whole panel to `height: 0` and the toggle went with it, so the log could be shut once and then not reopened: the only other way back is the `Log` chip in the canvas toolbar, which is itself rendered only while a pipeline is selected. The resize grip is hidden while it is shut — there is nothing to resize, and a `row-resize` cursor over a collapsed panel invites a drag that does nothing. `pipeline-log-panel` / `data-open` is what a test reads |
