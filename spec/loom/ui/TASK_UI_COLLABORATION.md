...
## Task: Wire the asset-detail Tasks tab to the real Task API and drop unsupported task fields

**Argumentation Summary:** The asset detail "Tasks" tab is fed by mock data —
[AssetDetail.tsx](../../../loom-ui/src/features/assetDetail/AssetDetail.tsx) loads tasks as
`Promise.resolve([] as Task[])` (line ~171), so the tab is always empty — and it renders
[TaskItem.tsx](../../../loom-ui/src/features/assetDetail/TaskItem.tsx) plus a task detail drawer
built on a local mock `Task` type carrying `status` (open/in_progress/review/done/blocked),
`assigneeId`, `dueDate`, and `tags`. **None of those fields exist in the REST task model** (see
scope note above); `status`/`due date`/asset links live only in the DB, and there is no REST
route to list tasks for an asset. This produces UI that cannot be backed by the API and displays
fields the server never returns.

**Improvement Summary:** Replace the mock asset-task model/tab with something the REST API can
support, and stop rendering task attributes the API does not expose.

```
Files: loom-ui/src/features/assetDetail/AssetDetail.tsx, TaskItem.tsx, loom-ui/src/types.ts.

Because REST has NO asset↔task listing/link, choose one:
  (a) Remove the asset-detail Tasks tab and mock task drawer entirely until the REST API adds
      an asset_task sub-resource; OR
  (b) Repoint the tab at the global task list (listTasks from api/tasks.ts) and render tasks
      using the REAL TaskResponse shape (title, description, priority only) — dropping status,
      assigneeId, dueDate, tags from TaskItem and the drawer.

If keeping any task rendering:
  - Use TaskResponse from api/tasks.ts, not the mock `Task` type in src/types.ts.
  - Map priority in the API's uppercase enum (LOW/MEDIUM/HIGH/CRITICAL); TaskItem currently keys
    priority colours on lowercase values that never match the API.

Also file a backend follow-up: the DOMAIN model (asset_task, annotation_task, task status, due
date) is not exposed by TaskEndpoint/TaskCreateRequest/TaskUpdateRequest/TaskResponse — the UI
cannot surface those until the REST layer adds them.
```

**References:**
- [AssetDetail.tsx](../../../loom-ui/src/features/assetDetail/AssetDetail.tsx), [TaskItem.tsx](../../../loom-ui/src/features/assetDetail/TaskItem.tsx)
- [tasks.ts](../../../loom-ui/src/api/tasks.ts), [TaskResponse.java](../../../loom-shared/rest-model/src/main/java/io/metaloom/loom/rest/model/task/TaskResponse.java)
- [DOMAIN.md](../DOMAIN.md) group 6

**Test Requirements:**
- If option (b): test the Tasks tab lists tasks from `listTasks` and renders priority chips from
  the uppercase enum; assert no status/assignee/dueDate elements render.
- If option (a): assert the Tasks tab and mock task drawer are removed and no mock task service
  is imported.
