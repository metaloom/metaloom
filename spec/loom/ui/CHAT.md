...
| Streaming endpoint (backend) | ✅ | `POST/DELETE /api/v1/chats/:uuid/stream` ([ChatStreamEndpoint](../../../loom/services/ai/src/main/java/io/metaloom/loom/ai/rest/ChatStreamEndpoint.java), SSE via [SseAgentEventSink](../../../loom/services/ai/src/main/java/io/metaloom/loom/ai/rest/SseAgentEventSink.java)); busy-guard (409), abort on disconnect, `ChatStreamEndpointTest`. |
| Agentic loop | ✅ | [loom/services/ai](../../../loom/services/ai): [AgentLoop](../../../loom/services/ai/src/main/java/io/metaloom/loom/ai/loop/AgentLoop.java) + [AgentService](../../../loom/services/ai/src/main/java/io/metaloom/loom/ai/AgentService.java) — transcript replay, in-process MCP tool dispatch, error-as-tool-result, turn limit, abort, persistence, auto-title. Config via `AiOptions` (`LOOM_AI_*`). |
| LLM access | ✅ | `genai-utils` compile-scope in `loom/services/ai`. `generateStreamWithTools` (streamed text/reasoning/tool calls) implemented for Ollama; vLLM falls back to the blocking path. True token streaming is opt-in via `LOOM_AI_STREAMING=true` ([StreamingTurnStreamer](../../../loom/services/ai/src/main/java/io/metaloom/loom/ai/loop/StreamingTurnStreamer.java)); default is turn-granular ([BlockingTurnStreamer](../../../loom/services/ai/src/main/java/io/metaloom/loom/ai/loop/BlockingTurnStreamer.java)). |
...
graph LR
    ...
    subgraph loom backend
        ...
        CSES --> AS[AgentService<br/>loom/services/ai]
        AS --> AL[AgentLoop]
...
- **`loom/services/ai`** (new Maven module, package `io.metaloom.loom.ai`): hosts the
