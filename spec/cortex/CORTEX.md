...
lifecycle, online/offline mode, node-kind registration, monitoring and the Loom
control channel.

**Cortex is a container, not a command line tool.** There is no picocli layer, no
subcommands and no flags — `CortexMain` starts the worker and configuration comes from
`cortex.yml` plus the environment. See [CONFIGURATION.md](CONFIGURATION.md).
...
|---|---|---|
| **Online** | Loom hostname resolvable from `LOOM_HOST` / `cortex.yml` (+ port) | `LoomControlChannel` opens the WebSocket, REGISTERs, receives source/node/segment tasks, syncs results |
| **Offline** | `CortexOptions.getLoom()` is `null` or its hostname is `null` | `CortexClientModule#restClient` returns `null`; no WebSocket, no tasks, no sync. Nothing drives work either — the worker idles |
...
Note that a worker with no `cortex.yml` **defaults to `localhost:7733`**
(`CortexOptionsLoader.generateDefaultConfig()`), so a plain `podman run` is *online*
against localhost, not offline. True offline requires clearing the `loom` section in
`cortex.yml`: a loaded file is never patched up with those defaults.
...
| `cli/` | `cortex-cli` | `CortexMain`, `CortexComponent`, `NodeCollectionModule`, `PipelineNodeFactoryModule`, `RegistryNodeRegistrar`; shaded jar. Named `cli` for historical reasons — it contains no command line |
...
| `CortexMain` | `io.metaloom.cortex.cli` (**cli**) | `main()`; builds the Dagger component, enforces `CORTEX_NODE_ID`, blocks in `cortex.run()`. Ignores any argument it is given |
| `CortexEnvOptions` | `io.metaloom.cortex.common.option` | Applies the `LOOM_*` / `CORTEX_*` variables onto `CortexOptions` |
| `CortexOptionsLoader` | `io.metaloom.cortex.common.option` | Loads `cortex.yml` (or the defaults), applies the environment, validates |
| `CortexComponent` | `io.metaloom.cortex.cli.dagger` (cli) | Dagger `@Component`; exposes `cortex()`, `options()`, `nodeFactory()`, `nodeRegistrar()` |
...
```
CortexMain.main(args)                          // args are ignored (warned about)
  ├── DaggerCortexComponent.builder().options(null).build()
  │     └── CortexClientModule.options(...) → CortexOptionsLoader.load()
  │           └── cortex.yml (or defaults) + CortexEnvOptions.applyEnv
  ├── require CORTEX_NODE_ID                   // else exit 2
  └── cortex.run()                             // blocking
```

### 4.2 No subcommands

There is exactly one thing to run, so there is nothing to select. `CortexMain` logs a
warning and continues when it is handed arguments, which keeps an image with a stale
`CMD … server start` bootable. Exit codes: `0` clean shutdown, `2` unusable
configuration (missing `CORTEX_NODE_ID`, unparseable environment), `10` a failure
after the worker went online.

### 4.3 Server mode

```mermaid
sequenceDiagram
    participant M as CortexMain
    ...
    M->>C: require CORTEX_NODE_ID; cortex.run()
...
`@Component(modules = { CortexBindModule, CortexMediaModule, PipelineNodeFactoryModule,
CortexClientModule, S3Module, CloudModule, MediaResolverModule })`
...
| `CortexClientModule` | core | `@Nullable LoomClient` (REST; `null` offline), `CortexOptions` (injected default, else `CortexOptionsLoader.load()` = `cortex.yml` + environment) |
...
`ProcessorRegistration`: `nodeId` (from `CORTEX_NODE_ID`, mandatory),
...
| Env | Default | Effect here |
|---|---|---|
| `LOOM_HOST` | `localhost` (no config file) | Absent/blank ⇒ offline; otherwise the WS endpoint host |
| `LOOM_PORT` | `7733` (no config file) | WS + REST port; `<= 0` ⇒ not configured |
| `CORTEX_MONITORING_PORT` | `8093` | Health/metrics/webhook server; also reported as the registration `host` suffix |
| `CORTEX_NODE_ID` | — | **Required**; must be unique and restart-stable. Missing ⇒ `CortexMain` exits 2 |
| `CORTEX_DRAIN_TIMEOUT_MS` | `30000` | Grace period in step 3 of the drain |
| `CORTEX_NODE_WHITELIST` | — (announces `registeredTypes()`) | Narrows what this worker accepts |
| `CORTEX_NODE_BLACKLIST` | — | Refusals; wins over the whitelist |
| `CORTEX_META_PATH` | `${user.home}/.cache/metaloom/cortex/meta` | Base for `filesystem-index/`, S3 index fallback |
...
- **No command line.** Anything a deployment must be able to set needs a variable in `CortexEnvOptions`; there is no flag to add. `MediaProcessor`/`FilesystemProcessor` survive without a caller — an offline batch mode would need a new entry point.
...
- **Container** runs `java … -jar cortex-cli.jar` with no arguments (Java 25, `--enable-native-access=ALL-UNNAMED`). See [BUILD.md](BUILD.md).
...
|---|---|
| `main()` | `cortex/cli/src/main/java/io/metaloom/cortex/cli/CortexMain.java` |
| Environment variables | `cortex/common/src/main/java/io/metaloom/cortex/common/option/CortexEnvOptions.java` |
...
- [x] Picocli removed — no CLI, no subcommands; env + `cortex.yml` only
