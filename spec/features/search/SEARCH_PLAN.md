...
| 6 | ⚠️ **The test-DB pool provisions in increments of 10 (max 60).** A 33-method class outruns it and fails with *"Got error from server {Unknown error}"*, which looks like a logic bug and is not. Keep test classes ≤15 methods. (Same cause as the intermittent `AssetEndpointTest` errors in a full `loom/core` run.) |
