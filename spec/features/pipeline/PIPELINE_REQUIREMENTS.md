...
| [../nodes/NODE_DATA_TYPES.md](../nodes/NODE_DATA_TYPES.md) | Ports, content types, cardinality, fan-out/gather |
...
| **R7** | Special pipeline nodes allow filtering of asset results (DateFilter, …) | ✅ | The eight unrunnable `filter-*` kinds and their nine `cortex/pipeline-core` classes are deleted; one `filter` kind replaces them, with a real `@StringKey` binding, dynamic per-bucket output ports and port-based routing (`PortSpec.selective`) — see [../nodes/NODE_DATA_TYPES.md §4.5 and §8.6](../nodes/NODE_DATA_TYPES.md). All six `filterBy` values are implemented: `LANGUAGE` (one LLM round trip); `MIME`, `SIZE` and `DATE`, which read the item's own metadata and hold no `LLMProvider`, so a filter-only pipeline needs no model backend; and `RATING` and `TAG`, which route on what reviewers recorded. |
...
| **Caching** | Node result caching wired in production | `[x]` — `LocalResultCache` in ~20 nodes. The unreachable `NodeCacheProvider` family was deleted 2026-08-02 rather than wired ([PIPELINE.md §7.3](PIPELINE.md)) |
| | Segment-scoped intermediate **artifact** cache (decode once per segment) | `[x]` — landed 2026-08-02, [PIPELINE.md §7.4](PIPELINE.md) |
