...
| **R7** | Special pipeline nodes allow filtering of asset results (DateFilter, …) | ✅ | The eight unrunnable `filter-*` kinds and their nine `cortex/pipeline-core` classes are deleted; one `filter` kind replaces them, with a real `@StringKey` binding, dynamic per-bucket output ports and port-based routing (`PortSpec.selective`) — see [NODE_DATA_TYPES.md §4.5 and §8.6](NODE_DATA_TYPES.md). All six `filterBy` values are implemented: `LANGUAGE` (one LLM round trip); `MIME`, `SIZE` and `DATE`, which read the item's own metadata and hold no `LLMProvider`, so a filter-only pipeline needs no model backend; and `RATING` and `TAG`, which route on what reviewers recorded. |
