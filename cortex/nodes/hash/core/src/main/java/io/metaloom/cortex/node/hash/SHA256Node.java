...
@NodeSpec(nodeId = "sha256", name = "SHA-256 Hash", icon = "fingerprint", category = NodeCategory.ANALYSIS,
	description = "Compute the SHA-256 hash of the media file.", defaultConcurrency = 4)
public class SHA256Node extends AbstractMediaNode<HashNodeOptions> {
	...
	@PortDoc(label = "Media", description = "The file whose bytes are digested")
	public static final InputPort<LoomMedia> IN_MEDIA = InputPort.one("media", ContentTypeRegistry.MEDIA_ANY, LoomMedia.class);

	@PortDoc(label = "SHA-256", description = "Lowercase hex SHA-256 digest over the whole file")
	...
	protected NodeResult compute(NodeContext<LoomMedia> ctx, AssetResponse asset) {
		...
		if (asset != null && asset.getHashes() != null && asset.getHashes().getSHA256() != null) {
