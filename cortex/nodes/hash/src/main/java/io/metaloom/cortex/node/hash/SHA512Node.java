...
public class SHA512Node extends AbstractMediaNode<HashPayload, HashNodeOptions> {
	...
	@Override
	protected NodeResult<HashPayload> compute(NodeContext<LoomMedia> ctx, AssetResponse asset) throws Exception {
		LoomMedia media = ctx.media();
		SHA512 hash = HashUtils.computeSHA512(media.file());
		media.setSHA512(hash);
		ctx.output(OUTPUT_SHA512, hash.toString());
		return ctx.origin(COMPUTED).next(HashPayload.of("SHA-512", hash.toString()));
	}
