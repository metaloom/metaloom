...
#: Last raised from 301 by ``ShareMethods`` (+23): eight owner-side calls on
#: ``/share-links`` and fifteen customer-side ones on ``/shares/{slug}``.
EXPECTED_JAVA_METHOD_COUNT = 324
...
PYTHON_ONLY = {
    ...
    "set_token",
    "token",
    # The share credential, mirroring the token pair above. Java declares these on
    # LoomHttpClient rather than in a *Methods interface, so the parser never sees
    # them - exactly as with setToken.
    "set_share_session_token",
    "share_session_token",
    "base_url",
}
...
EXTRA_KWARGS = {
    ...
    "upload_user_avatar": {"filename": "x.bin"},
    "upload_my_avatar": {"filename": "x.bin"},
}
...
class ServerRouteParityTest(unittest.TestCase):
    ...
    def test_asset_sub_resources_reject_a_sha512(self):
        ...
        asset. Nothing nested under it -- no tags, tasks, detections, detection
        reviews or reactions -- has a hash form, so the client refuses rather than
        building a path that 404s.
