...
description: "Experiments in generating perceptual video fingerprints, stage by stage, from a source frame to the hex value that identifies the footage."
...
.Source frame
image::stage-1.jpg[The unmodified source frame at 512x512: a cartoon squirrel climbing a tree trunk, with a green meadow and pink blossom behind it]
...
.Reduced
image::stage-2.jpg[The same frame in greyscale at 16x16, blurred and normalised — the tree is a dark band on the left, the sky a bright blur on the right, and no detail smaller than the trunk survives]
...
.Normalized
image::stage-3.jpg[The 16x16 greyscale frame after normalisation: the same shapes with the bright areas pulled back and the contrast spread across the full grey range]
...
.Stacked
image::stage-4.jpg[The frame stacked with its predecessor: the two 16x16 greyscale images averaged into one, so moving detail smears and the static trunk and horizon stay put]
...
.Normalized
image::stage-5.jpg[The stacked frame after a second normalisation, which pulls back the bright spots stacking created]
...
.Result
image::stage-6.jpg[The finished 16x16 mask reduced to one bit per pixel: pure black and white blocks, which read straight off as the bits of the fingerprint]
