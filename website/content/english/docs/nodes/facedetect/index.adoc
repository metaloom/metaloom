...
description: "Finds the faces in your media and sorts them into the people it believes it saw."
---
...
Every decision is also **signed and dated**: the group records which member of your team confirmed or
rejected it, and when. That record is kept apart from the processing history the node writes, so it
survives every later run — six months on you can still answer "who said this is Anna, and when?".
Naming a face is a decision about a real person, and it is not one that should be traceable only to
"the pipeline". A group nobody has looked at carries no reviewer at all, so "unreviewed" and "reviewed
by the software" can never be confused.
...
Both are set on the node itself in the pipeline editor — select the face-detection node and edit
**Cluster Radius** and **Min Cluster Size** in the settings panel. They belong to that node, so a
pipeline can run two face-detection nodes at different strictness, and changing one leaves every
other pipeline alone. The editor holds Cluster Radius to its valid range of `0` to `2`.
