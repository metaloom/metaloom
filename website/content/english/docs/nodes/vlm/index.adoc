...
++++
<div class="ml-nodeviz" data-nodeviz='{"kind":"vlm","applies":"Image","badge":"GPU vision endpoint","persist":"asset_json_comp + asset_node_result ledger","configs":[{"name":"olmOCR preset","note":"the default when no prompts are configured","inputs":[{"t":"image","l":"document page","d":"scaled to maxImageDim"}],"outputs":[{"t":"text","l":"vlm_result_olmocr","d":"markdown transcription"},{"t":"json","l":"page observations","d":"language, rotation, is_table"}]},{"name":"custom prompts","note":"one output per configured prompt id","inputs":[{"t":"image","l":"image","d":"scaled to maxImageDim"}],"outputs":[{"t":"text","l":"vlm_result_caption","d":"responseFormat TEXT"},{"t":"json","l":"vlm_result_fields","d":"responseFormat JSON"}]}]}'></div>
++++
...
| Applies to | Image
| Inputs | None
| Output keys | `vlm_result_{promptId}` (String) — for the shipped preset, `vlm_result_olmocr`
...
| `prompts` | Named tasks to run against the image, keyed by prompt id
|===
