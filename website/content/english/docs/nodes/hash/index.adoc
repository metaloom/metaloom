...
| Output ports | `hash` — one port per kind, typed `hash/md5`, `hash/sha256`, `hash/sha512` or `hash/chunk`. The type is what downstream nodes match on, so a link:../loom/[Loom Sync] or link:../dedup/[dedup] node never has to name the node that produced it
