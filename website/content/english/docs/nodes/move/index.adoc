...
**It keeps the original unless you say otherwise.** By default the node copies and leaves the source
in place — and says so: the `flag` port reads `COPIED` rather than `MOVED`. Removing the original is
something you switch on, and even then it only happens after the destination has been checked to hold
the same content.
...
| Original file | Keep | Keep it, or remove it once the destination has been verified
