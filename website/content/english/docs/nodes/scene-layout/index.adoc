...
++++
<div class="ml-nodeviz" data-nodeviz='{"kind":"scene-layout","applies":"Images with depth + detections","badge":"No model required","persist":"asset_json_comp (schemaType scene-layout)","configs":[{"name":"faces in a photo","note":"depthmap + facedetect upstream","inputs":[{"t":"json","l":"depthmap_meta","d":"depth map from the depthmap node"},{"t":"json","l":"detections","d":"boxes from face detection"}],"outputs":[{"t":"json","l":"scene_layout_result","d":"objects, bands, relations, phrases"},{"t":"number","l":"scene_layout_object_count"},{"t":"number","l":"scene_layout_relation_count"}]}]}'></div>
++++
...
| Inputs | A depth map from link:../depthmap/[Depth Map], plus bounding boxes from link:../facedetect/[Face Detection] (or any other detector)
| Output keys | `scene_layout_result` (JSON String), `scene_layout_object_count` (Number), `scene_layout_relation_count` (Number)
...
|===
| Option | Meaning
| `depthNodeId` | Which upstream node provides the depth map. Defaults to `depthmap`
| `detectionSources` | Which upstream detector nodes provide boxes, in priority order. Defaults to `facedetect`
| `allowLoomFallback` | Read detections back from the server when no upstream node supplied them. On by default
...
This node reads other nodes' work, so both have to be in your pipeline ahead of it:

. link:../depthmap/[Depth Map] — required. Put it in the **same affinity group** as this node, so
  both run on the same machine.
. A detector — link:../facedetect/[Face Detection] today. At present that means the node relates
  faces to faces; once object detection is available it will relate people to cars and everything
  else without any change here, because it does not care which detector produced a box.
