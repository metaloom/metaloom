...
++++
<div class="ml-nodeviz" data-nodeviz='{"kind":"s3-sink","applies":"Output node","badge":"needs S3 credentials","persist":"bucket object + a new asset per file","configs":[{"name":"publish artifacts","note":"default: picks up what upstream produced","inputs":[{"t":"path","l":"thumbnail / depth map / audio","d":"produced by an upstream node"}],"outputs":[{"t":"path","l":"s3:// object","d":"one per file"},{"t":"flag","l":"asset","d":"registered in Loom"}]},{"name":"archive originals","note":"option: includeSource","inputs":[{"t":"media","l":"the media item","d":"as it entered the pipeline"}],"outputs":[{"t":"path","l":"s3:// object","d":"the original, archived"}]}]}'></div>
++++
...
| Inputs | The media item, plus any upstream file outputs
| Output keys | `s3_sink_flag`, `s3_sink_count`, `s3_sink_result`
...
By default, **everything the upstream nodes produced** — the sink recognises file outputs
automatically, so putting it after a thumbnail node just works with no configuration.
...
NOTE: Images produced by a *Script* node must always be listed explicitly. Their output names are
chosen by whoever wrote the script, so they cannot be recognised automatically.
...
| `artifacts` | Explicit `nodeId:outputKey` list. Empty means "detect automatically"
| `autoDiscover` | Whether to detect file outputs when no explicit list is given
