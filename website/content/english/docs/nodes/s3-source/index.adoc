...
++++
<div class="ml-nodeviz" data-nodeviz='{"kind":"s3-source","applies":"Source node","badge":"needs S3 credentials","persist":"nothing — it feeds the graph","configs":[{"name":"bucket scan","note":"options: bucket, prefix","inputs":[{"t":"path","l":"bucket + prefix","d":"listed, metadata only"}],"outputs":[{"t":"path","l":"uri","d":"s3://bucket/key per changed object"},{"t":"flag","l":"state","d":"NEW / MODIFIED / DELETED"},{"t":"flag","l":"source","d":"s3"}]},{"name":"bucket notifications","note":"option: useEvents","inputs":[{"t":"flag","l":"change events","d":"pushed by the bucket"}],"outputs":[{"t":"path","l":"uri","d":"only the reported objects"},{"t":"flag","l":"state","d":"NEW / MODIFIED / DELETED"},{"t":"flag","l":"source","d":"s3"}]}]}'></div>
++++
...
| Output keys | `uri`, `bucket`, `key`, `source` (= `s3`), `state` for the item currently flowing through the graph
