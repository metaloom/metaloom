...
| Input ports | `media`, plus `data` for whatever you connect from upstream
...
| `upstream` | What arrived on the connected inputs, keyed by node and port: `upstream['whisper']['transcript']`
...
const transcript = JSON.parse(upstream['whisper']['transcript']);
