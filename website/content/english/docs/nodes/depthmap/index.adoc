...
++++
<div class="ml-nodeviz" data-nodeviz='{"kind":"depthmap","applies":"Images","badge":"Depth sidecar","persist":"Local depthmap_bin cache + node-result ledger","configs":[{"name":"relative (default)","note":"orders objects front-to-back","inputs":[{"t":"media","l":"image","d":"any still image"}],"outputs":[{"t":"path","l":"depthmap_path","d":"16-bit PNG on the worker"},{"t":"json","l":"depthmap_meta","d":"model, dimensions, statistics"},{"t":"label","l":"depthmap_flag","d":"DONE / FAILED"}]},{"name":"metric","note":"mode = METRIC, adds a range in metres","inputs":[{"t":"media","l":"image"}],"outputs":[{"t":"path","l":"depthmap_path"},{"t":"json","l":"depthmap_meta","d":"includes min/max metres"},{"t":"label","l":"depthmap_flag"}]}]}'></div>
++++
...
| Inputs | The image asset itself — no upstream node required
| Output keys | `depthmap_flag` (String), `depthmap_path` (String), `depthmap_meta` (JSON String)
...
The generated map is stored on the worker that produced it. If you chain this node with
link:../scene-layout/[Scene Layout], put both in the **same affinity group** in your pipeline so
they run on the same machine — otherwise the second node cannot find the map.
