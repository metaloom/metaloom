...
Translation runs on a language model you already operate — any Ollama or OpenAI-compatible endpoint —
...
| Requirements | An **Ollama or OpenAI-compatible endpoint** serving the configured model
...
| `model` | The model asked to translate (default `gemma2:27b`). Recorded with every result
| `ollamaUrl` | Base URL of the backend (default `http://127.0.0.1:11434`)
| `providerType` | `OLLAMA` or `VLLM` for an OpenAI-compatible endpoint
