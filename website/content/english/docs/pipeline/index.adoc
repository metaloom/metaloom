...
image::debug-detail-faces.png[Face detections drawn over the frame, with a thumbnail per detected face,role=img-fluid]
...
Note the ten detections over two people. Face detection reports one entry per face *per sampled
frame*, so `face_count` counts detections rather than distinct people — and the thumbnails make
that visible at a glance in a way the number alone does not.
