...
public class AuthenticationOptions implements Option {
	...
	public void validate(OptionErrors errors) {
		// The keystore password is required to open/create the JWT keystore. It is generated by
		// LoomOptionsLoader#generateDefaultConfig when no configuration file exists.
		errors.notBlank("keystorePassword", keystorePassword)
			.min("tokenExpirationTime", tokenExpirationTime, 1);

		if (mcpAuthEnabled) {
			errors.notBlank("mcpAuthAllowedOrigins", mcpAuthAllowedOrigins);
		}

		errors.nested("oauth2", oauth2);
	}
